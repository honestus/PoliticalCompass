...
sc = SocialnessCalculator('C:/Users/onest/Documents/TextAn/tesi/code/data/utils/SocialnessNorms_DiveicaPexmanBinney2021.csv', min_max=(0,1), expand_lexicon=True, limit_expansion=True)
vad = NRCVad('C:/Users/onest/Documents/TextAn/tesi/code/data/utils/NRC-VAD-Lexicon/NRC-VAD-Lexicon/NRC-VAD-Lexicon.txt', expand_lexicon=True, limit_expansion=True, min_max=(-1,1))
...
def extract_sentiment(text, include_subjectivity=True):
    vader_sent = vader_analyzer.polarity_scores(text)['compound']
...
def extract_socialness(text):
    ...
    socialness_values = sc.calculate_socialness(text, strong_socialness_threshold=0.63, fillna=False, lemmatize=True)
    try:
...
def extract_vad(text):   
    vad_scores = vad.calculate_vad_scores(text, fillna=False, lemmatize=True)
...
def extract_emotions(tokenized_text):
    ...
    emotions_scores = nrc.load_raw_text(text).raw_emotion_scores
...
def extract_populism(text): ###!!! UNDONE, SO FAR IT ASSIGNS RANDOM VALUES!!!
    import random

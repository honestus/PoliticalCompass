...
def get_standard_settings_for_replacement(**kwargs) -> dict[str]:
    """
...
def replace_features_from_raw_text(text: str, sorted_features: list[dict]) -> str:
    """
...
def replace_features_from_tokens(tokens: list[str], sorted_features: list[dict], features_names_to_skip: list[str]) -> list[str]:
    """
...
def reset_features_from_replaced_text(text: str, sorted_features: list[dict], columns_to_remove: list[str], **kwargs) -> str:
    """
...
def reset_features_from_replaced_tokens(tokens: list[str], sorted_features: list[dict], features_names_to_remove: list[str], **kwargs) -> list[str]:
...
def replace_features_in_text(row: pd.Series,
    ...
    """
    curr_vars = get_standard_settings_for_replacement(**locals())
    ...
    if not revert:
        ...
        if tokenized:
            ...
            return replace_features_from_tokens(orig_text, sorted_features=sorted_features, features_names_to_skip=curr_features_names_to_remove)
        
        else: ###if orig_text is a string (not tokenized text in input)
            return replace_features_from_raw_text(text=orig_text, sorted_features=sorted_features)

    else:
        sorted_features = (f for f in 
                       get_sorted_features(row=row, **get_standard_settings_for_replacement(columns_to_replace=columns_to_replace+columns_to_remove,
                                                                                           columns_to_remove=[],
                                                                                           revert=True))
                          )
        if tokenized:
            curr_features_names_to_remove = [v for k,v in default_config.single_feature_names_dict.items() if k in columns_to_remove]
            return reset_features_from_replaced_tokens(tokens=orig_text, sorted_features=sorted_features, 
            ...
            return reset_features_from_replaced_text(text=orig_text, sorted_features=sorted_features,

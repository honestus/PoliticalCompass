...
def get_emoticons_pattern(emoticons_list: list[str], pattern:int =0) -> re.Pattern:
    ...
    pattern_regex = ''
    if pattern==1:
        pattern_regex = re.compile(rf"(?:^|(?<=\s))({emoticon_joined_string})(?=\s|$)") ### ONLY MATCHES SINGLE EMOTICONS 
    elif pattern==2:
        pattern_regex = re.compile(rf"(?:^|(?<=\s))({emoticon_joined_string})+(?=\s|$)") ### MATCHES MULTIPLE EMOTICONS BUT ONLY KEEPS TRACK OF THE LAST ONE
    
    else:
        pattern_regex = regex.compile(r"(?:^|(?<=\s|" + emoticon_joined_string + "))" + "(" + emoticon_joined_string + ")"\
                                  +r"(?=\s|$|" + emoticon_joined_string +")") ### MATCHES AND REPLACES ALL OF THE EMOTICONS (notice that a string such as "Nice:):P" will match :P as it matches any emoticons either followed/preceded by another emoticon or a space/end of line
...
def get_badwords_pattern(bad_words_list: list[str]) -> re.Pattern:
    ...
    rf"(?:^|(?<=[\s{re.escape(string.punctuation)}]))"
    rf"({badwords_joined_string})"
    r"(?=\W|$)"
)
